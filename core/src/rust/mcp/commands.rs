...
pub async fn get_mcp_tools_config(state: State<'_, AppState>) -> Result<Vec<MCPToolConfig>, String> {
    ...
    tools.push(MCPToolConfig {
        id: mcp::TOOL_INTERACT.to_string(),
        name: "寸止".to_string(),
        description: "智能代码审查交互工具，支持预定义选项、自由文本输入和图片上传".to_string(),
        enabled: config.mcp_config.tools.get(mcp::TOOL_INTERACT).copied().unwrap_or(true),
    ...
    // 记忆管理工具 - 始终存在，无配置选项
    tools.push(MCPToolConfig {
        id: mcp::TOOL_MEMORY.to_string(),
        name: "记忆管理".to_string(),
        description: "全局记忆管理工具，用于存储和管理重要的开发规范、用户偏好和最佳实践".to_string(),
        enabled: config.mcp_config.tools.get(mcp::TOOL_MEMORY).copied().unwrap_or(false),
        can_disable: true,
    ...
    // 代码搜索工具 - 始终存在，有配置选项
    tools.push(MCPToolConfig {
        id: mcp::TOOL_SEARCH.to_string(),
        name: "代码搜索".to_string(),
        description: "基于查询在特定项目中搜索相关的代码上下文，支持语义搜索和增量索引".to_string(),
        enabled: config.mcp_config.tools.get(mcp::TOOL_SEARCH).copied().unwrap_or(false),
        can_disable: true,
...
pub async fn set_mcp_tool_enabled(
...
) -> Result<(), String> {
    {
        ...
        // 检查工具是否可以禁用
        if tool_id == mcp::TOOL_INTERACT && !enabled {
