...
use crate::mcp::tools::{InteractionTool, MemoryTool};
use crate::mcp::types::{InteractRequest, MemoryRequest};
use crate::mcp::utils::errors::invalid_params_error;
use crate::mcp::tools::unified_store::{init_global_store, init_global_watcher};

/// 确保搜索系统只初始化一次 (虽然现在只初始化存储和监听器，保留 Once 以防并发调用)
static INIT_ONCE: Once = Once::new();

/// 初始化 MCP 系统（存储和监听器）
fn ensure_system_initialized() {
    INIT_ONCE.call_once(|| {
