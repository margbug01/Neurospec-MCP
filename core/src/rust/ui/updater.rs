...
async fn download_and_install_update_impl(app: &AppHandle, update_info: &UpdateInfo) -> Result<(), String> {
    ...
    // 创建临时目录
    let temp_dir = std::env::temp_dir().join("neurospec_update");
...
async fn install_from_archive(file_path: &PathBuf) -> Result<(), String> {
    ...
    // 创建临时解压目录
    let temp_dir = std::env::temp_dir().join("neurospec_extract");
    if temp_dir.exists() {
...
fn find_executable_in_dir(dir: &PathBuf) -> Result<PathBuf, String> {
    ...
    for file in &files {
        if let Some(file_name) = file.file_name().and_then(|n| n.to_str()) {
            ...
            // 查找名为 "NeuroSpec" 的可执行文件
            if file_name == "NeuroSpec" || file_name == "neurospec" ||
               file_name == "NeuroSpec.exe" || file_name == "neurospec.exe" ||
               file_name.starts_with("NeuroSpec") && !file_name.ends_with(".tar.gz") {
