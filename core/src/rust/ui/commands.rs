...
use crate::mcp::tools::interaction::{InteractRecord, get_interact_history, search_interact_history, clear_interact_history};

/// 获取 interact 历史记录
#[tauri::command]
pub async fn get_interact_history_cmd(count: Option<usize>) -> Result<Vec<InteractRecord>, String> {
    get_interact_history(count).map_err(|e| format!("获取历史记录失败: {}", e))
}

/// 搜索 interact 历史记录
#[tauri::command]
pub async fn search_interact_history_cmd(query: String) -> Result<Vec<InteractRecord>, String> {
    search_interact_history(&query).map_err(|e| format!("搜索历史记录失败: {}", e))
}

/// 清空 interact 历史记录
#[tauri::command]
pub async fn clear_interact_history_cmd() -> Result<(), String> {
    clear_interact_history().map_err(|e| format!("清空历史记录失败: {}", e))
}
...
/// 获取嵌入配置文件路径
fn get_embedding_config_path() -> PathBuf {
    dirs::home_dir()
...
/// 测试嵌入连接
#[tauri::command]
pub async fn test_embedding_connection_cmd(config: EmbeddingConfigFrontend) -> Result<TestConnectionResult, String> {
    use reqwest::Client;
    use std::time::Duration;
    
    // 简单的连接测试：向 API 发送一个请求
    ...
    if response.status().is_success() {
        Ok(TestConnectionResult {
            success: true,
            message: format!("✅ 连接成功 ({})", config.provider),
        })
    } else {
        ...
        Ok(TestConnectionResult {
            success: false,
            message: format!("❌ API 返回 {}: {}", status, error_text),
        })
    }
}
