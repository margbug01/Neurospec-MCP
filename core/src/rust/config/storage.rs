...
fn get_standalone_config_path() -> Result<PathBuf> {
    ...
    let new_config_dir = base_config_dir.join("neurospec");
    let old_config_dir = base_config_dir.join("cunzhi");
    
    // 迁移旧配置（如果存在）
    if old_config_dir.exists() && !new_config_dir.exists() {
        log::info!("Migrating config from cunzhi to neurospec...");
        if let Err(e) = fs::rename(&old_config_dir, &new_config_dir) {
            log::warn!("Failed to migrate config directory: {}. Copying instead...", e);
            // 如果重命名失败，尝试复制
            if let Err(copy_err) = copy_dir_all(&old_config_dir, &new_config_dir) {
                log::error!("Failed to copy config directory: {}", copy_err);
            } else {
                log::info!("Config migrated successfully (via copy)");
            }
        } else {
    ...
    Ok(new_config_dir.join("config.json"))
}
