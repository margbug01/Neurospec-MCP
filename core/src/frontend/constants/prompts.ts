...
export const PROMPT_SECTIONS = {
  // Zhi 工具（NeuroSpec）提示词
  zhi: {
    base: `- 只能通过MCP \`NeuroSpec\` 对我进行询问，禁止直接询问或结束任务询问`,
    detail: `NeuroSpec工具使用细节：
- 需求不明确时使用 \`NeuroSpec\` 询问澄清，提供预定义选项
- 在有多个方案的时候，需要使用 \`NeuroSpec\` 询问，而不是自作主张
- 在有方案/策略需要更新时，需要使用 \`NeuroSpec\` 询问，而不是自作主张
- 即将完成请求前必须调用 \`NeuroSpec\` 请求反馈
- 在没有明确通过使用 \`NeuroSpec\` 询问并得到可以完成任务/结束时，禁止主动结束对话/请求`,
  } as PromptSection,
...
export const DEFAULT_MCP_TOOLS: MCPToolConfig[] = [
  {
    id: 'zhi',
    name: 'Zhi 智能审查工具',
    description: '智能代码审查交互工具（NeuroSpec）',
    enabled: true,
    canDisable: false,
