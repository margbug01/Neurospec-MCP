...
## 2. `codebase-retrieval` —— 代码检索 MCP（理解代码的第一选择）
这是主要的代码理解与搜索工具，请视为**代码理解的第一选择**。
...
**不适合的用途**（这时更适合用 IDE 原生精确查找 / grep / file view）：
...
# Codebase-Retrieval MCP 使用规范
该节是对代码检索 MCP 的专门补充，指导你在 NeuroSpec 流程里什么时候、怎么用。
